...
namespace EMBC.Utilities.S3
{
    public class S3Provider : IS3Provider
    {
        ...
        private async Task<string> UploadStorageItem(UploadFileCommand cmd, CancellationToken cancellationToken)
        {
            ...
            var folder = cmd.Folder == null ? string.Empty : $"{cmd.Folder}/";
            ...
            if (cmd.File != null && file != null)
            {
                ...
                var response = await amazonS3Client.PutObjectAsync(request, cancellationToken);
        ...
        {
            var dir = folder == null ? string.Empty : $"{folder}/";
            ...
            var response = await amazonS3Client.GetObjectAsync(request, ct);
            response.EnsureSuccess();
            ...
            //get tagging
            var tagResponse = await amazonS3Client.GetObjectTaggingAsync(
        ...
        private async Task<string> UpdateTags(UpdateTagsCommand cmd, CancellationToken cancellationToken)
        {
            ...
            var response = await amazonS3Client.PutObjectTaggingAsync(request, cancellationToken);
            response.EnsureSuccess();
            if (cmd.Key != null)
            {
                return cmd.Key;
