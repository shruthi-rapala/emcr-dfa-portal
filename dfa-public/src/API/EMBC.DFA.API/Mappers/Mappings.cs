    ...
    public class Mappings : AutoMapper.Profile
    {
        public Mappings()
        {
            ...
            CreateMap<dfa_appapplication, CurrentApplication>()
                ...
                //.ForMember(d => d.Stage, opts => opts.MapFrom(s => !string.IsNullOrEmpty(s.dfa_applicationstatusportal) ?
                //    (Convert.ToInt32(s.dfa_applicationstatusportal) == Convert.ToInt32(ApplicationStages.Draft) ? null : GetEnumDescription(ApplicationSubStages.Pending))
                //    : null))
                .ForMember(d => d.Stage, opts => opts.MapFrom(s => !string.IsNullOrEmpty(s.dfa_applicationstatusportal) ?
                    (s.dfa_applicationstatusportal.Contains(ApplicationStages.Draft.ToString()) ? null : GetEnumDescription(ApplicationSubStages.Pending))
                    : null))
